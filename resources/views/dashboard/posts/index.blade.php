...
<div class="sm:px-6 w-full">
  <div class="px-4 md:px-10 py-4 md:py-7">
      <div class="flex items-center justify-between">
          <p tabindex="0" class="dark:text-white focus:outline-none text-base sm:text-lg md:text-xl lg:text-2xl font-bold leading-normal text-gray-800">Posts</p>
                  ...
                  <tr tabindex="0" class="focus:outline-none h-16 border border-gray-100 rounded">
                    ...
                    <td class="">
                      <div class="flex items-center pl-5">
                        <p class="text-base font-medium leading-none text-gray-700 mr-2 dark:text-white">{{ $post->title }}</p>
                      </div>
                    </td>
                    <td class="pl-24">
                      <div class="flex items-center">
                        <p class="text-sm leading-none gap-1 text-gray-600 ml-2 flex dark:text-white"><i data-feather="tag" class="h-4 w-4"></i>{{ $post->category->name }}</p>
                      </div>
                    </td>
                    <td class="pl-5">
                      <button class="py-3 px-3 text-sm focus:outline-none leading-none text-red-700 bg-red-100 rounded">{{ $post->created_at->diffForHumans() }}</button>
                    </td>
                    <td class="pl-4">
                      ...
                      <div class="relative px-5 pt-2">
                        ...
                        <div class="dropdown-content bg-white shadow w-24 absolute z-30 right-0 mr-6 hidden">
                          <div class="flex flex-col">
                            <a href="/dashboard/posts/{{ $post->slug }}/edit" tabindex="0" class="focus:outline-none focus:text-indigo-600 text-xs w-full hover:bg-indigo-700 py-4 px-8 cursor-pointer hover:text-white">
                              <p>Edit</p>
                            </a>
                            <form action="/dashboard/posts/{{ $post->slug }}" method="POST" class="border-0">
                              @method('delete')
                              @csrf
  ...
  function confirmDelete(event, form) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        // Mengirim permintaan delete ke server dengan metode DELETE
        var url = form.action;
          ...
          .then((response) => {
            if (response.status === 204) {
              ...
              Swal.fire(
                ...
                location.reload(); // Memuat ulang halaman setelah gagal menghapus data
              });
            }
          })
          .catch((error) => {
